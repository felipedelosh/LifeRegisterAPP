...
public class graphicsProccesorBox extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(littleBoxvalues.size()>0){
            ...
            for(int i=0;i<=littleBoxvalues.size()-1;i++){
                ...
                if(i==0){
                    float x0 = (float) 0;
                    float y0 = (float) h;
                    float x1 = separationScale * (float) (i+1);
                    float y1 = (float) h - ((float) h * ((float) littleBoxvalues.get(i)/(float) maxValue));
                    canvas.drawLine(x0,y0,x1,y1,brush);
                }else{
                    float x0 = separationScale * i;
                    float y0 = (float) h - ((float) h * ((float) littleBoxvalues.get(i-1)/(float) maxValue));
                    float x1 = separationScale * (float) (i+1);
                    float y1 = (float) h - ((float) h * ((float) littleBoxvalues.get(i)/(float) maxValue));;
                    canvas.drawLine(x0,y0,x1,y1,brush);
                }
        ...
        if(bigBoxValues.size() > 0){
            ...
            //Paint medium
            canvas.drawText("$"+Math.round(maxValue/2), w*(float)0.8,h/2, brush);
            //paint min
            canvas.drawText("$0", w*(float)0.8,h-50, brush);

            //I need a separation scale to graph
            float separationScale = w/bigBoxValues.size();
            ...
            for(int i=0;i<=bigBoxValues.size()-1;i++){
                ...
                if(i==0){
                    ...
                    float y0 = (float) h;
                    float x1 = separationScale * (float) (i+1);
                    float y1 = (float) h - ((float) h * ((float) bigBoxValues.get(i)/(float) maxValue));
                    canvas.drawLine(x0,y0,x1,y1,brush);
                }else{
                    float x0 = separationScale * i;
                    float y0 = (float) h - ((float) h * ((float) bigBoxValues.get(i-1)/(float) maxValue));
                    float x1 = separationScale * (float) (i+1);
                    float y1 = (float) h - ((float) h * ((float) bigBoxValues.get(i)/(float) maxValue));;
                    canvas.drawLine(x0,y0,x1,y1,brush);
                }
